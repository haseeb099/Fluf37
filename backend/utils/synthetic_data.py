...
def get_demo_failures() -> List[FailureRecord]:
    ...
    return [
        FailureRecord(
            id="fail_001",
            title="Circular payment loop — Acme Corp",
            description="March 2026: 3-hop circular payment Acme→Supplier X→Shell→Acme. Loss $79k.",
            source_ids=["bank", "crm", "erp"],
            affected_entities=["Acme Corp", "Supplier X Ltd", "Nexus Holdings LLC", "Treasury cash pool"],
            dollar_loss=79000,
        ...
        FailureRecord(
            id="fail_002",
            title="ACME order-book spoofing & RSI divergence",
            description="Order book spoofing on ACME: RSI divergence preceded 8% drop.",
            source_ids=["trading", "news"],
            affected_entities=["ACME position", "Equities desk", "Market risk limits"],
            dollar_loss=21500,
        ...
        FailureRecord(
            id="fail_003",
            title="Customer–shareholder overlap (Acme deal)",
            description="Customer-shareholder overlap undetected on Acme deal until audit.",
            source_ids=["crm", "erp"],
            affected_entities=["Acme Corp", "James Chen (CFO)", "Credit committee"],
            dollar_loss=45000,
